...
def add_fields(token, app_token, table_id, fields):
    ...
    for field_name, field_type in fields.items():
        ...
        elif field_type == 'date': type_code = 5
...
def update_config(table_id):
    config_path = os.path.join(CONFIG_DIR, 'sources.yaml')
    with open(config_path, 'r', encoding='utf-8') as f:
        config = yaml.safe_load(f)
    
    config['api_keys']['vocab_table_id'] = table_id
    
    with open(config_path, 'w', encoding='utf-8') as f:
        yaml.dump(config, f, allow_unicode=True, default_flow_style=False)
    print(f"✅ Updated sources.yaml with vocab_table_id: {table_id}")

def main():
    print("🚀 Setting up Vocab Table...")
    ...
    table_id = create_table(tenant_token, app_token, "User Vocabulary V3")
    ...
    if table_id:
        add_fields(tenant_token, app_token, table_id, {
            ...
            "Saved Date": "text",  # Changed to Text
            "Review Count": "text" # Changed to Text
        })
        print(f"\n✅ User Vocabulary Table Ready! ID: {table_id}")
        update_config(table_id)
